...
inline uint32_t pypilot_available_modes(bool compass_available,
    ...
    if (compass_available) {
        mask |= pypilot_mode_mask_compass;
    }
    if (gps_available) {
        if (gps_and_nav_modes || !nav_available) {
            mask |= pypilot_mode_mask_gps;
        }
        if (nav_available) {
            mask |= pypilot_mode_mask_nav;
        }
    }
    if (wind_available) {
        mask |= pypilot_mode_mask_wind;
    }
    if (true_wind_available) {
        mask |= pypilot_mode_mask_true_wind;
    }
    return mask;
}
...
inline Real pypilot_mode_heading(PypilotMode mode,
    ...
    switch (mode) {
    case PypilotMode::true_wind:
        return pypilot_resolv(true_wind_compass_offset_deg - compass_heading_deg);
    case PypilotMode::wind:
        return pypilot_resolv(wind_compass_offset_deg - compass_heading_deg);
    case PypilotMode::gps:
    case PypilotMode::nav:
        return pypilot_resolv_to(compass_heading_deg + gps_compass_offset_deg, Real(180));
    case PypilotMode::compass:
